(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{constructor(){this.events={},this.state={theme:localStorage.getItem(`fes_theme`)||`light`,user:JSON.parse(localStorage.getItem(`fes_user`))||null,bookings:JSON.parse(localStorage.getItem(`fes_bookings`))||null,notifications:JSON.parse(localStorage.getItem(`fes_notifications`))||null,onboardingComplete:localStorage.getItem(`fes_onboarding_complete`)===`true`},this.applyTheme(this.state.theme)}subscribe(e,t){return this.events[e]||(this.events[e]=[]),this.events[e].push(t),()=>{this.events[e]=this.events[e].filter(e=>e!==t)}}publish(e,t){this.events[e]&&this.events[e].forEach(e=>e(t))}setState(e,t){this.state[e]=t,e===`user`?localStorage.setItem(`fes_user`,JSON.stringify(t)):e===`bookings`?localStorage.setItem(`fes_bookings`,JSON.stringify(t)):e===`notifications`?localStorage.setItem(`fes_notifications`,JSON.stringify(t)):e===`onboardingComplete`&&localStorage.setItem(`fes_onboarding_complete`,t.toString()),this.publish(e,t),this.publish(`state_changed`,this.state)}getState(){return this.state}toggleTheme(){let e=this.state.theme===`light`?`dark`:`light`;localStorage.setItem(`fes_theme`,e),this.state.theme=e,this.applyTheme(e),this.publish(`theme`,e),this.publish(`state_changed`,this.state)}applyTheme(e){document.documentElement.setAttribute(`data-theme`,e)}login(e,t=`User`){let n={email:e,name:t.split(`@`)[0],avatar:null,loggedInAt:new Date().toISOString()};this.setState(`user`,n)}logout(){this.setState(`user`,null)}},t=new class{constructor(){this.routes=[],this.container=null,this.activeView=null,window.addEventListener(`hashchange`,()=>this.handleRouting())}register(e,t){let n=e.replace(/\/:([^/]+)/g,`/([^/]+)`).replace(/\//g,`\\/`),r=(e.match(/:([^/]+)/g)||[]).map(e=>e.slice(1));this.routes.push({path:e,regex:RegExp(`^#${n}$`),paramNames:r,component:t})}init(e){if(this.container=document.getElementById(e),!this.container){console.error(`Mount container #${e} not found.`);return}this.handleRouting()}navigate(e){window.location.hash=e}handleRouting(){let[t,n]=(window.location.hash||`#/onboarding`).split(`?`),r=e.getState();if(!r.onboardingComplete&&t!==`#/onboarding`){this.navigate(`#/onboarding`);return}if(r.onboardingComplete&&!r.user&&t!==`#/auth`&&t!==`#/onboarding`){this.navigate(`#/auth`);return}if(r.user&&(t===`#/auth`||t===`#/onboarding`)){this.navigate(`#/dashboard`);return}let i=null,a=null;for(let e of this.routes)if(i=t.match(e.regex),i){a=e;break}if(!a){console.warn(`Route ${t} not found. Fallback to dashboard.`),this.navigate(`#/dashboard`);return}let o={};if(a.paramNames.length>0&&i&&a.paramNames.forEach((e,t)=>{o[e]=decodeURIComponent(i[t+1])}),n){let e=new URLSearchParams(n);for(let[t,n]of e.entries())o[t]=n}this.mount(a.component,o)}async mount(e,t){this.activeView&&typeof this.activeView.destroy==`function`&&this.activeView.destroy();let n=new e(t);this.activeView=n,this.container.innerHTML=`<div class="app-view-container animate-fade-in no-scrollbar"><div style="padding: 40px; text-align: center; color: var(--color-text-tertiary);">Loading...</div></div>`;try{let e=await n.render();this.container.innerHTML=e,typeof n.afterRender==`function`&&n.afterRender(),window.lucide&&typeof window.lucide.createIcons==`function`&&window.lucide.createIcons()}catch(e){console.error(`Render error:`,e),this.container.innerHTML=`
    ...
    `}afterRender(){this.themeToggle=document.getElementById(`bookings-theme-toggle`),this.bookingCards=document.querySelectorAll(`.booking-card`),this.themeToggle.addEventListener(`click`,()=>{e.toggleTheme();let t=e.getState().theme===`light`?`moon`:`sun`;this.themeToggle.innerHTML=`<i data-lucide="${t}"></i>`,window.lucide.createIcons()}),this.bookingCards.forEach(e=>{e.addEventListener(`click`,n=>{if(n.target.closest(`a`)||n.target.closest(`button`))return;let r=e.getAttribute(`data-id`);this.expandedBookingId===r?this.expandedBookingId=null:this.expandedBookingId=r,t.handleRouting()})}),document.querySelectorAll(`.bottom-nav-bar .nav-item`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.currentTarget.getAttribute(`data-route`);t.navigate(n)})})}},_=class{constructor(e={}){this.params=e}async render(){let t=e.getState().notifications||[];return`
      <!-- Header with back button -->
      <header class="header-nav">
    ...
    `}afterRender(){this.themeSwitch=document.getElementById(`profile-theme-switch`),this.btnLogout=document.getElementById(`btn-logout`),this.themeSwitch.addEventListener(`change`,()=>{e.toggleTheme(),t.handleRouting()}),this.btnLogout.addEventListener(`click`,()=>{e.logout(),t.navigate(`#/auth`)}),document.querySelectorAll(`.bottom-nav-bar .nav-item`).forEach(e=>{e.addEventListener(`click`,e=>{let n=e.currentTarget.getAttribute(`data-route`);t.navigate(n)})})}},v={xmlns:`http://www.w3.org/2000/svg`,width:24,height:24,viewBox:`0 0 24 24`,fill:`none`,stroke:`currentColor`,"stroke-width":2,"stroke-linecap":`round`,"stroke-linejoin":`round`},y=([e,t,n])=>{let r=document.createElementNS(`http://www.w3.org/2000/svg`,e);return Object.keys(t).forEach(e=>{r.setAttribute(e,String(t[e]))}),n?.length&&n.forEach(e=>{let t=y(e);r.appendChild(t)}),r},b=(e,t={})=>y([`svg`,{...v,...t},e]),x=e=>{for(let t in e)if(t.startsWith(`aria-`)||t===`role`||t===`title`)return!0;return!1},S=(...e)=>e.filter((e,t,n)=>!!e&&e.trim()!==``&&n.indexOf(e)===t).join(` `).trim(),C=e=>e.replace(/^([A-Z])|[\s-_]+(\w)/g,(e,t,n)=>n?n.toUpperCase():t.toLowerCase()),w=e=>{let t=C(e);return t.charAt(0).toUpperCase()+t.slice(1)},T=e=>Array.from(e.attributes).reduce((e,t)=>(e[t.name]=t.value,e),{}),E=e=>typeof e==`string`?e:!e||!e.class?``:e.class&&typeof e.class==`string`?e.class.split(` `):e.class&&Array.isArray(e.class)?e.class:``,D=(e,{nameAttr:t,icons:n,attrs:r})=>{let i=e.getAttribute(t);if(i==null)return;let a=n[w(i)];if(!a)return console.warn(`${e.outerHTML} icon name was not found in the provided icons object.`);let o=T(e),s=x(o)?{}:{"aria-hidden":`true`},c={...v,"data-lucide":i,...s,...r,...o},l=E(o),u=E(r),d=S(`lucide`,`lucide-${i}`,...l,...u);d&&Object.assign(c,{class:d});let f=b(a,c);return e.parentNode?.replaceChild(f,e)},O=[[`path`,{d:`M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2`}]],k=[[`path`,{d:`m12 19-7-7 7-7`}],[`path`,{d:`M19 12H5`}]],A=[[`path`,{d:`M5 12h14`}],[`path`,{d:`m12 5 7 7-7 7`}]],j=[[`path`,{d:`M10.268 21a2 2 0 0 0 3.464 0`}],[`path`,{d:`M17 17H4a1 1 0 0 1-.74-1.673C4.59 13.956 6 12.499 6 8a6 6 0 0 1 .258-1.742`}],[`path`,{d:`m2 2 20 20`}],[`path`,{d:`M8.668 3.01A6 6 0 0 1 18 8c0 2.687.77 4.653 1.707 6.05`}]],M=[[`path`,{d:`M10.268 21a2 2 0 0 0 3.464 0`}],[`path`,{d:`M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326`}]],N=[[`path`,{d:`M10 12h4`}],[`path`,{d:`M10 8h4`}],[`path`,{d:`M14 21v-3a2 2 0 0 0-4 0v3`}],[`path`,{d:`M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2`}],[`path`,{d:`M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16`}]],P=[[`path`,{d:`M8 2v4`}],[`path`,{d:`M16 2v4`}],[`rect`,{width:`18`,height:`18`,x:`3`,y:`4`,rx:`2`}],[`path`,{d:`M3 10h18`}],[`path`,{d:`m14 14-4 4`}],[`path`,{d:`m10 14 4 4`}]],F=[[`path`,{d:`M8 2v4`}],[`path`,{d:`M16 2v4`}],[`rect`,{width:`18`,height:`18`,x:`3`,y:`4`,rx:`2`}],[`path`,{d:`M3 10h18`}]],I=[[`path`,{d:`M20 6 9 17l-5-5`}]],L=[[`path`,{d:`m6 9 6 6 6-6`}]],R=[[`path`,{d:`m15 18-6-6 6-6`}]],z=[[`path`,{d:`m18 15-6-6-6 6`}]],B=[[`path`,{d:`M21.801 10A10 10 0 1 1 17 3.335`}],[`path`,{d:`m9 11 3 3L22 4`}]],V=[[`path`,{d:`M12 13v8`}],[`path`,{d:`M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242`}],[`path`,{d:`m8 17 4-4 4 4`}]],ne=[[`path`,{d:`M10.733 5.076a10.744 10.744 0 0 1 11.205 6.575 1 1 0 0 1 0 .696 10.747 10.747 0 0 1-1.444 2.49`}],[`path`,{d:`M14.084 14.158a3 3 0 0 1-4.242-4.242`}],[`path`,{d:`M17.479 17.499a10.75 10.75 0 0 1-15.417-5.151 1 1 0 0 1 0-.696 10.75 10.75 0 0 1 4.446-5.143`}],[`path`,{d:`m2 2 20 20`}]],H=[[`path`,{d:`M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0`}],[`circle`,{cx:`12`,cy:`12`,r:`3`}]],U=[[`path`,{d:`M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z`}],[`path`,{d:`M14 2v5a1 1 0 0 0 1 1h5`}],[`path`,{d:`M10 9H8`}],[`path`,{d:`M16 13H8`}],[`path`,{d:`M16 17H8`}]],W=[[`path`,{d:`M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8`}],[`path`,{d:`M3 10a2 2 0 0 1 .709-1.528l7-6a2 2 0 0 1 2.582 0l7 6A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z`}]],G=[[`path`,{d:`m10 17 5-5-5-5`}],[`path`,{d:`M15 12H3`}],[`path`,{d:`M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4`}]],K=[[`path`,{d:`m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7`}],[`rect`,{x:`2`,y:`4`,width:`20`,height:`16`,rx:`2`}]],q=[[`path`,{d:`M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0`}],[`circle`,{cx:`12`,cy:`10`,r:`3`}]],J=[[`path`,{d:`M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z`}]],Y=[[`path`,{d:`M20.985 12.486a9 9 0 1 1-9.473-9.472c.405-.022.617.46.402.803a6 6 0 0 0 8.268 8.268c.344-.215.825-.004.803.401`}]],X=[[`path`,{d:`M12 22V12`}],[`path`,{d:`M20.27 18.27 22 20`}],[`path`,{d:`M21 10.498V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.729l7 4a2 2 0 0 0 2 .001l.98-.559`}],[`path`,{d:`M3.29 7 12 12l8.71-5`}],[`path`,{d:`m7.5 4.27 8.997 5.148`}],[`circle`,{cx:`18.5`,cy:`16.5`,r:`2.5`}]],Z=[[`path`,{d:`M13 2a9 9 0 0 1 9 9`}],[`path`,{d:`M13 6a5 5 0 0 1 5 5`}],[`path`,{d:`M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384`}]],re=[[`path`,{d:`M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384`}]],ie=[[`circle`,{cx:`6`,cy:`19`,r:`3`}],[`path`,{d:`M9 19h8.5a3.5 3.5 0 0 0 0-7h-11a3.5 3.5 0 0 1 0-7H15`}],[`circle`,{cx:`18`,cy:`5`,r:`3`}]],ae=[[`path`,{d:`M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915`}],[`circle`,{cx:`12`,cy:`12`,r:`3`}]],oe=[[`path`,{d:`M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z`}],[`path`,{d:`m9 12 2 2 4-4`}]],se=[[`circle`,{cx:`12`,cy:`12`,r:`4`}],[`path`,{d:`M12 2v2`}],[`path`,{d:`M12 20v2`}],[`path`,{d:`m4.93 4.93 1.41 1.41`}],[`path`,{d:`m17.66 17.66 1.41 1.41`}],[`path`,{d:`M2 12h2`}],[`path`,{d:`M20 12h2`}],[`path`,{d:`m6.34 17.66-1.41 1.41`}],[`path`,{d:`m19.07 4.93-1.41 1.41`}]],Q=[[`path`,{d:`M10 11v6`}],[`path`,{d:`M14 11v6`}],[`path`,{d:`M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6`}],[`path`,{d:`M3 6h18`}],[`path`,{d:`M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2`}]],ce=[[`path`,{d:`m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3`}],[`path`,{d:`M12 9v4`}],[`path`,{d:`M12 17h.01`}]],le=[[`path`,{d:`M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2`}],[`circle`,{cx:`9`,cy:`7`,r:`4`}],[`line`,{x1:`19`,x2:`19`,y1:`8`,y2:`14`}],[`line`,{x1:`22`,x2:`16`,y1:`11`,y2:`11`}]],ue=[[`path`,{d:`M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2`}],[`circle`,{cx:`12`,cy:`7`,r:`4`}]],de=[[`path`,{d:`M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.106-3.105c.32-.322.863-.22.983.218a6 6 0 0 1-8.259 7.057l-7.91 7.91a1 1 0 0 1-2.999-3l7.91-7.91a6 6 0 0 1 7.057-8.259c.438.12.54.662.219.984z`}]],fe=[[`path`,{d:`M18 6 6 18`}],[`path`,{d:`m6 6 12 12`}]],pe=[[`path`,{d:`M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z`}]],$=({icons:e={},nameAttr:t=`data-lucide`,attrs:n={},root:r=document,inTemplates:i}={})=>{if(!Object.values(e).length)throw Error(`Please provide an icons object.
If you want to use all the icons you can import it like:
 \`import { createIcons, icons } from 'lucide';
lucide.createIcons({icons});\``);if(r===void 0)throw Error("`createIcons()` only works in a browser environment.");if(Array.from(r.querySelectorAll(`[${t}]`)).forEach(r=>D(r,{nameAttr:t,icons:e,attrs:n})),i&&Array.from(r.querySelectorAll(`template`)).forEach(r=>$({icons:e,nameAttr:t,attrs:n,root:r.content,inTemplates:i})),t===`data-lucide`){let t=r.querySelectorAll(`[icon-name]`);t.length>0&&(console.warn(`[Lucide] Some icons were found with the now deprecated icon-name attribute. These will still be replaced for backwards compatibility, but will no longer be supported in v1.0 and you should switch to data-lucide`),Array.from(t).forEach(t=>D(t,{nameAttr:`icon-name`,icons:e,attrs:n})))}};window.lucide={createIcons:()=>$({icons:{Wrench:de,Settings:ae,Activity:O,AlertTriangle:ce,ArrowLeft:k,MessageSquare:J,ArrowRight:A,ShieldCheck:oe,LogIn:G,Eye:H,EyeOff:ne,X:fe,UserPlus:le,CheckCircle:B,Bell:M,Home:W,Calendar:F,Phone:re,User:ue,CalendarX:P,ChevronDown:L,ChevronLeft:R,ChevronUp:z,Check:I,FileText:U,Trash2:Q,Mail:K,Moon:Y,Sun:se,Building2:N,PhoneCall:Z,MapPin:q,PackageSearch:X,BellOff:j,UploadCloud:V,Zap:pe,Route:ie}})},t.register(`#/onboarding`,n),t.register(`#/auth`,c),t.register(`#/dashboard`,f),t.register(`#/service/:id`,p),t.register(`#/booking`,m),t.register(`#/bookings`,g),t.register(`#/notifications`,_),t.register(`#/support`,ee),t.register(`#/profile`,te),document.addEventListener(`DOMContentLoaded`,()=>{let e=document.getElementById(`app`);e&&(e.innerHTML=`<div id="app-simulator"></div>`),t.init(`app-simulator`)});